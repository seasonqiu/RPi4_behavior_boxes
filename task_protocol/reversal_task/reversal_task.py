# python3: reversal_task.py
"""
author: tian qiu
date: 2024-02-02
name: reversal_task.py
goal: reversal task for reversal conditioning
description:
    reversal task for reversal conditioning
...
class PullLeverGetSugar(object):
    ...
    def __init__(self, **kwargs):  # name and session_info should be provided as kwargs
        ...
        self.transitions = [
            ...
            ['restart', ['cue_state', 'reward_available'], 'inter_trial_interval']
        ]
    ...
    def run(self):
        ...
        if self.event_name is "lever_pulled":
            # print("EVENT NAME !!!!!! " + self.event_name)
            if self.state == "inter_trial_interval" or self.state == "reward_available":
                pass
            else:
                self.early_error = True
                self.error_repeat = True
                self.restart()
        if self.state == "inter_trial_interval":
            pass
    ...
    def enter_standby(self):
        ...
        if self.early_error:
            self.error_list.append("early_error")
            logging.info(";" + str(time.time()) + ";[error];early_error;" + str(self.error_repeat))
